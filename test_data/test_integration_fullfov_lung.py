#!/usr/bin/env python3
"""
Full Integration Test: Image Features + RNA Expression
=======================================================
...
Data: Xenium Human Lung 2 FOV (test dataset)
"""
...
# CONFIG
# ────────────────────────────────────────────────────────────────────────────
FULL_FOV = True                       # Process entire image (no crop)
CROP_Y0, CROP_X0 = 0, 0              # Unused when FULL_FOV=True
CROP_SIZE = None                      # Unused when FULL_FOV=True
...
print("=" * 70)
print("FULL FOV INTEGRATION: Image Features + RNA Expression")
print("=" * 70)
t_start = time.time()
...
# Take first Z-slice (DAPI, the relevant channel for nuclear morphology)
image_full = img[0] if img.ndim == 3 else img
print(f"  Full image: {image_full.shape}, dtype={image_full.dtype}")

# Full FOV: use entire image
y1, x1 = 0, 0
y2, x2 = image_full.shape
image = image_full
print(f"  Full FOV: ({image.shape[1]}×{image.shape[0]} px)")
...
# --- 1a. Filter cells in FOV & rasterize ---
print("\nFiltering cells in FOV region...")
xs_all = (nuclei_df["vertex_x"].values * MICRONS_TO_PX).astype(int)
ys_all = (nuclei_df["vertex_y"].values * MICRONS_TO_PX).astype(int)

# Full FOV: cells within image bounds
in_fov = ((xs_all >= 0) & (xs_all < image.shape[1]) &
          (ys_all >= 0) & (ys_all < image.shape[0]))
fov_cell_ids = nuclei_df["cell_id"].iloc[np.where(in_fov)[0]].unique()
print(f"  Cells in FOV: {len(fov_cell_ids)}")

if len(fov_cell_ids) > 3000:
    print(f"  ⚠ Large FOV ({len(fov_cell_ids)} cells), may be slow.")

nuclei_fov = nuclei_df[nuclei_df["cell_id"].isin(fov_cell_ids)].copy()

print("Rasterizing cell masks...")
mask = np.zeros(image.shape, dtype=np.int32)

xs_fov = (nuclei_fov["vertex_x"].values * MICRONS_TO_PX).astype(int)
ys_fov = (nuclei_fov["vertex_y"].values * MICRONS_TO_PX).astype(int)
nuclei_fov = nuclei_fov.assign(px_x=xs_fov, px_y=ys_fov)

grouped = nuclei_fov.groupby("cell_id")
n_cells_fov = len(grouped)
cell_centroids_px = {}
...
for cell_id, group in grouped:
    if len(group) < 3:
        skipped += 1
        continue
    xs = np.clip(group["px_x"].values, 0, image.shape[1] - 1)
    ys = np.clip(group["px_y"].values, 0, image.shape[0] - 1)
...
# Precompute compartment boundary distance and intensity gradient ONCE (not per cell)
ep_edge = skimage.morphology.dilation(epithelium, skimage.morphology.disk(1)) ^ epithelium
if ep_edge.any():
    edt = scipy.ndimage.distance_transform_edt(~ep_edge)
else:
    edt = None
...
comp_intensity_grad = []
for _, row in morph_df.iterrows():
    cy = int(min(row['centroid-0'], image.shape[0] - 1))
    cx = int(min(row['centroid-1'], image.shape[1] - 1))
    if necrosis[cy, cx]:
    ...
    comp_boundary_dist.append(float(edt[cy, cx]) if edt is not None else np.nan)
    comp_intensity_grad.append(float(grad_mag[cy, cx]))
...
# ════════════════════════════════════════════════════════════════════════════
# PHASE 7: Leiden Clustering on Multimodal Space
# ════════════════════════════════════════════════════════════════════════════
print("\n" + "─" * 70)
print("PHASE 7: Leiden Clustering on Multimodal Data")
print("─" * 70)

import scanpy as sc
sc.settings.verbosity = 0  # suppress scanpy chatter
...
for idx, (res, label) in enumerate([('leiden_r0.3', 'r=0.3'),
    ...
    for clust in sorted(adata.obs[res].unique()):
        ...
        ax.scatter(adata.obsm['X_pca'][mask, 0],
                   ...
                   s=5, label=f'{clust}', alpha=0.7)
    ax.set_title(f'Leiden (res={label})')
    ax.set_xlabel(f'PC1 ({pca_mm.explained_variance_ratio_[0]*100:.1f}%)')
    ax.set_ylabel(f'PC2 ({pca_mm.explained_variance_ratio_[1]*100:.1f}%)')
    ax.legend(markerscale=3, fontsize=8)

fig.suptitle('Multimodal PCA — Leiden Clusters', fontsize=14, y=1.02)
plt.tight_layout()
fig.savefig(OUT / "plots" / "leiden_clusters.png", dpi=150, bbox_inches='tight')
plt.close(fig)
print("  leiden_clusters.png ✓")

# Spatial map of clusters
fig, ax = plt.subplots(1, 1, figsize=(8, 8))
for clust in sorted(adata.obs['leiden'].unique()):
    mask = adata.obs['leiden'] == clust
    ax.scatter(adata.obsm['spatial'][mask, 0],
               adata.obsm['spatial'][mask, 1],
               s=5, label=f'Cluster {clust}', alpha=0.7)
ax.set_title('Leiden Clusters — Spatial Map')
ax.set_xlabel('X (µm)')
ax.set_ylabel('Y (µm)')
ax.legend(markerscale=3, fontsize=8)
plt.tight_layout()
fig.savefig(OUT / "plots" / "leiden_spatial_map.png", dpi=150, bbox_inches='tight')
plt.close(fig)
print("  leiden_spatial_map.png ✓")
...
print(f"  Saved: leiden_clusters.csv ({len(cluster_df)} cells, "
      f"cluster range: 0..{n_clusters-1})")
...
│  Leiden clusters:      {n_clusters} (res=0.5)                                │
│                                                              │
│  Output:              {OUT}                │
